...
describe('Template - TemplateHeaderNavigation.js', () => {
  ...
  describe('Component functions', () => {
    ...
    test('previousTemplate() - Attempt to get the previous template in our list and update the URL', () => {
      wrapper = shallow(<TemplateHeaderNavigation {...props} history={historyMock} />)
      const instance = wrapper.instance()
      instance.previousTemplate({ preventDefault () {}, stopPropagation () {} })

      expect(historyMock.push.mock.calls.length).toBe(1)
    })

    test('nextTemplate() - Attempt to get the next template in our list and update the URL', () => {
      wrapper = shallow(<TemplateHeaderNavigation {...props} history={historyMock} />)
      const instance = wrapper.instance()
      instance.nextTemplate({ preventDefault () {}, stopPropagation () {} })
