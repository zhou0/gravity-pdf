...
describe('Template - TemplateListItem.js', () => {
  ...
  describe('Component functions', () => {
    ...
    test('maybeShowDetailedTemplate() - Check if the Enter key is pressed and not focused on a button then display the template details page', () => {
      const props = { template: { id: 'blank-slate', template: 'Blank Slate' } }
      const e = {
        keyCode: 13,
        target: { className: '' }
      ...
      const showDetailedTemplate = jest.spyOn(wrapper.instance(), 'showDetailedTemplate')
      wrapper.instance().maybeShowDetailedTemplate(e)

      expect(showDetailedTemplate).toHaveBeenCalledTimes(1)
      expect(historyMock.push.mock.calls.length).toBe(1)
    })

    test('showDetailedTemplate() - Update the URL to show the PDF template details page', () => {
      const props = { template: { id: 'blank-slate', template: 'Blank Slate' } }
      wrapper = shallow(<TemplateListItem {...props} history={historyMock} />)
      wrapper.instance().showDetailedTemplate()
