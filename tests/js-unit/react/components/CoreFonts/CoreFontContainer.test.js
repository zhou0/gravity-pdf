...
describe('CoreFonts - CoreFontContainer.js', () => {
  ...
  describe('Component functions', () => {
    ...
    test('triggerFontDownload() - request GitHub for font names & trigger font download', () => {
      const newWrapper = shallow(
        <CoreFontContainer
          fontList={[]}
  ...
  describe('Run Lifecycle methods', () => {
    ...
    test('componentDidMount() - Check for /downloadCoreFonts redirect URL and run the installer', () => {
      ...
      const triggerFontDownload = jest.spyOn(wrapper.instance(), 'triggerFontDownload')
      wrapper.instance().componentDidMount()

      expect(triggerFontDownload).toHaveBeenCalledTimes(1)
      expect(wrapper.state('ajax')).toBe(true)
      expect(getFilesFromGitHubMock.mock.calls.length).toBe(1)
    })

    test('componentWillReceiveProps() - Load current font list', done => {
      const nextProps = {
        fontList: fontList,
        location: {
          pathname: ''
    ...
    test('componentWillReceiveProps() - Check for /downloadCoreFonts redirect URL and run the installer', () => {
      const nextProps = {
        fontList: [],
    ...
    test('componentWillReceiveProps() - Load current hash history location & retry font list', done => {
      const nextProps = {
        fontList: [],
        retry: fontList,
    ...
    test('componentWillReceiveProps() - Load error if something went wrong', () => {
      const nextProps = {
        fontList: [],
        location: { pathname: '' },
    ...
    test('componentWillReceiveProps() - Set ajax/loading false if request download is finished', () => {
      const nextProps = {
        fontList: [],
        location: { pathname: '' },
