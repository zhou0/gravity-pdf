...
# The `-u 33:33` flag tells Docker to run the command as a particular user and
# prevents permissions errors. See: https://github.com/WordPress/gutenberg/pull/8427#issuecomment-410232369
docker-compose $DOCKER_COMPOSE_FILE_OPTIONS run --rm -u 33:33 $CLI db reset --yes --quiet
docker-compose $DOCKER_COMPOSE_FILE_OPTIONS run --rm -u 33:33 $CLI core install --title="$SITE_TITLE" --admin_user=admin --admin_password=password --admin_email=test@test.com --skip-email --url=$HOST_URL --quiet
...
if [ "$CURRENT_URL" != "$HOST_URL" ]; then
	docker-compose $DOCKER_COMPOSE_FILE_OPTIONS run --rm -u 33:33 $CLI option update home "$HOST_URL" --quiet
	docker-compose $DOCKER_COMPOSE_FILE_OPTIONS run --rm -u 33:33 $CLI option update siteurl "$HOST_URL" --quiet
fi

# Configure site constants.
echo -e $(status_message "Configuring site constants...")
docker-compose $DOCKER_COMPOSE_FILE_OPTIONS run --rm -u 33:33 $CLI config set WP_DEBUG $WP_DEBUG --raw --type=constant --quiet
WP_DEBUG_RESULT=$(docker-compose $DOCKER_COMPOSE_FILE_OPTIONS run -T --rm -u 33:33 $CLI config get --type=constant --format=json WP_DEBUG)
echo -e $(status_message "WP_DEBUG: $WP_DEBUG_RESULT...")

docker-compose $DOCKER_COMPOSE_FILE_OPTIONS run --rm -u 33:33 $CLI config set SCRIPT_DEBUG $SCRIPT_DEBUG --raw --type=constant --quiet
SCRIPT_DEBUG_RESULT=$(docker-compose $DOCKER_COMPOSE_FILE_OPTIONS run -T --rm -u 33:33 $CLI config get --type=constant --format=json SCRIPT_DEBUG)
echo -e $(status_message "SCRIPT_DEBUG: $SCRIPT_DEBUG_RESULT...")

# Setup additional users
docker-compose $DOCKER_COMPOSE_FILE_OPTIONS run --rm -u 33:33 $CLI user create editor editor@test.com --role=editor --user_pass=password --quiet
